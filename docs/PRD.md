...
- **Accepting or rejecting suggested edits.** The generally-available Google Docs API does not support resolving suggestions programmatically. This server lists them (closing the visibility gap); acting on them remains a human step in the Docs UI. Update, July 2026: Google now documents `AcceptSuggestionRequest`/`RejectSuggestionRequest`/`DeleteSuggestionRequest` (and suggestion creation via `writeMode=SUGGEST`) under the Workspace Developer Preview Program only — the stable v1 surface strips the fields for non-preview callers (verified 2026-07-17 against the discovery document and a live probe). Revisit verified accept/reject tools when the feature reaches GA; until then this remains a known limitation, not a planned feature.
